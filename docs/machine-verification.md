...
- `pnpm check`: lint, type checks, 30 unit tests, and production build passed.
...
The intro now runs for eight seconds. The original opening retains its timing; the launcher releases at 5.5 seconds and lands at 6.8 seconds. The longer-throw follow-up passed 120 browser checks on the full run; the unchanged finale screenshot passed a targeted rerun against its original baseline after reverting an incidental snapshot refresh. The launcher-to-key gap is 1.85 world units, with the launcher moved clear of the last domino and connected by a release lever. The active domino passes motion immediately to the next at contact; earlier pieces follow the moving contact faces instead of waiting on independent rotation curves. Tests check for uninterrupted movement and separation from the release lever and cup; a separating-axis check verifies adjacent solids at 4,001 sampled times. Unit checks verify release from the cup, continuous landing, compression only after impact, one settling bounce, and deterministic reset. The browser stage sequence is now marble → dominoes → launch → key → complete.
